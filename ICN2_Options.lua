...
local function formatBiasLabel(needLabel, sliderPos, decayMult, readOnly)
    local L = ICN2.L
    local label = ICN2:L("DECAY_SLIDER_FORMAT", needLabel, sliderPos, decayMult)
    if readOnly then
        label = label .. "  " .. L["DECAY_SLIDER_PRESET"]
    end
    return label
end
...
function ICN2:BuildOptions() -- Called once on ADDON_LOADED to construct the options UI. The frame is hidden by default and shown when the user clicks "Options" in the slash command or via Interface Options.
    ...
    optFrame.TitleText:SetText(ICN2.COLOR.ADDON .. L["ADDON_NAME"] .. ICN2.COLOR.RESET .. " – " ..
        L["ADDON_TITLE"] .. "  " .. ICN2.COLOR.GRAY .. L["ADDON_VERSION"] .. ICN2.COLOR.RESET)
    ...
    -- ══════════ GENERAL TAB ══════════════════════════════════════════════════
    makeLabel(panelGeneral, L["SECTION_HUD"], 14, -6, 1, 0.8, 0)
    ...
    makeCheckbox(panelGeneral, L["OPT_HUD_LOCK"], 14, -52,
        function() return ICN2DB.settings.hudLocked end,
        function(v) ICN2DB.settings.hudLocked = v; ICN2:LockHUD(v) end)

    makeLabel(panelGeneral, L["SECTION_THEME"], 200, -6, 1, 0.8, 0)

    local THEMES = ICN2.HUD_THEME_LIST or {
        { id = "smooth", label = L["THEME_SMOOTH"] },
        { id = "blocky", label = L["THEME_BLOCKY"] },
        { id = "folk", label = L["THEME_FOLK"] },
        { id = "necromancer", label = L["THEME_NECROMANCER"] },
    }
    ...
    local function themeLabel()
        ...
        if ICN2.GetHUDTheme then
            return (ICN2:GetHUDTheme(current).label) or L["THEME_SMOOTH"]
        end
        for _, t in ipairs(THEMES) do
            if t.id == current then return t.label end
        end
        return L["THEME_SMOOTH"]
    end
    ...
    makeLabel(panelGeneral, L["SECTION_LABELS"], 200, -56, 1, 0.8, 0)

    local LABEL_MODES = {
        { id = "none",       label = L["LABEL_MODE_NONE"] },
        { id = "percentage", label = L["LABEL_MODE_PERCENTAGE"] },
        { id = "number",     label = L["LABEL_MODE_NUMBER"] },
        { id = "both",       label = L["LABEL_MODE_BOTH"] },
    }
    ...
    local function labelModeLabel()
        ...
        return L["LABEL_MODE_PERCENTAGE"]
    end
    ...
    makeLabel(panelGeneral, L["SECTION_IMMERSION"], 14, -225, 1, 0.8, 0)

    makeCheckbox(panelGeneral, L["OPT_FREEZE_OFFLINE"] .. "  " .. L["OPT_FREEZE_OFFLINE_DESC"], 14, -245,
    ...
    fdLabel:SetText(L["OPT_FOOD_DRINK_AUTO"])
    fdLabel:SetWidth(400)

    makeSeparator(panelGeneral, 14, -302, 358)

    makeLabel(panelGeneral, L["SECTION_EMOTES"], 14, -310, 1, 0.8, 0)
    ...
    makeLabel(panelGeneral, L["SECTION_MANUAL_RESTORE"], 14, -456, 1, 0.8, 0)
    ...
    makeLabel(panelGeneral, L["SECTION_MANUAL_DEPLETE"], 14, -508, 1, 0.8, 0)
    ...
    -- ══════════ DECAY TAB ════════════════════════════════════════════════════
    makeLabel(panelDecay, L["OPT_DECAY_PRESET"], 14, -6, 1, 0.8, 0)

    local presets = { "fast", "medium", "slow", "realistic", "custom" }
    local presetLabels = {
        fast      = L["PRESET_FAST"],
        medium    = L["PRESET_MEDIUM"],
        slow      = L["PRESET_SLOW"],
        realistic = L["PRESET_REALISTIC"],
    ...
    help1:SetText(
        L["OPT_DECAY_HELP_1"] .. "\n" ..
        ICN2:L("OPT_DECAY_HELP_2", ICN2.CUSTOM_DECAY_MULTIPLIER_MAX or 30))

    makeSeparator(panelDecay, 14, -108, 400)

    makeLabel(panelDecay, L["OPT_PER_NEED_BIAS"], 14, -118, 1, 0.8, 0)

    makeDecayBiasSlider(panelDecay, "hunger",  L["DECAY_SLIDER_HUNGER"],  14, -142)
    makeDecayBiasSlider(panelDecay, "thirst",  L["DECAY_SLIDER_THIRST"],  14, -192)
    makeDecayBiasSlider(panelDecay, "fatigue", L["DECAY_SLIDER_FATIGUE"], 14, -242)
